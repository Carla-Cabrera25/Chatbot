...
const AI_API_TOKEN = (process.env.APIAI_TOKEN) ?
...
function callThreadAPI(messageData) {
  request({
    ...
    if (!error && response.statusCode == 200) {
      ...
      console.error("Failed calling ThreadSettings API", response.statusCode, response.statusMessage, body.error);
    }
  });
}
